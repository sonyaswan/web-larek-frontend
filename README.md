...
* `handleResponse`- обрабатывает ответ от API. Если ответ успешный, он возвращает JSON-объект. Если ответ неуспешный, он возвращает ошибку.
...
Используется класс `EventEmitter` (класс описан в разделе базовых классов).

Описание событий:
```
'items:changed' //вывод каталога с продуктами на страницу
'preview:changed' //открытие модального окна с выбранной карточкой
...
'order:start' // начало заполнения заказа
